...
fn generate_scenario(
    ...
    // Generate Uncontrolled Agents (Fixed Tracks: either horizontal or vertical)
    for _ in 0..num_uncontrolled {
        for _ in 0..100 { // retry limit
            let is_horizontal = rng.random_bool(0.5);
            let mut track = vec![];
            if is_horizontal {
                let y = rng.random_range(0..size);
                for x in 0..size { track.push(Vector2::new(x as f32, y as f32)); }
            } else {
                let x = rng.random_range(0..size);
                for y in 0..size { track.push(Vector2::new(x as f32, y as f32)); }
            }
            
            let start = (track[0].y.round() as usize, track[0].x.round() as usize);
            if !occupied_cells.contains(&start) {
                for pos in &track {
                    occupied_cells.insert((pos.y.round() as usize, pos.x.round() as usize));
                }
    ...
    for _ in 0..num_controlled {
        ...
        let mut found = false;
        for &s in &possible_cells {
            if !occupied_cells.contains(&s) {
                for &e in &possible_cells {
                    if s != e {
                        starts.push(s);
                        ends.push(e);
...
fn replan(
    ...
    let mut rng = StdRng::seed_from_u64(seed + (sim_time * 1234.0) as u64);
    
    for obs in agents.iter_mut() {
        if obs.uncontrolled {
            let mut track = vec![];
            let actual_current_wp_idx = obs.follower.as_mut().map(|f| f.get_semantic_waypoint().trajectory_index).unwrap_or(0);
            
            let start_wp_idx = match mode {
                InformationMode::Oracle => actual_current_wp_idx,
                InformationMode::Nominal => actual_current_wp_idx,
                InformationMode::Noisy => {
                    let noise = rng.random_range(-1..=1);
                    (actual_current_wp_idx as i64 + noise).max(0) as usize
                }
            };

            for j in start_wp_idx..obs.original_path.len() {
    ...
    for obs in agents.iter_mut() {
        if obs.uncontrolled {
            ...
            for j in (actual_current_wp_idx + 1)..obs.original_path.len() {
...
fn run_simulation(size: usize, num_uncontrolled: usize, num_controlled: usize, seed: u64, mode: InformationMode) -> SimulationResult {
    let (mut agents, grid) = generate_scenario(size, num_uncontrolled, num_controlled, seed);
    if agents.len() < (num_uncontrolled + num_controlled) { return SimulationResult::default(); }
    ...
    while sim_time < MAX_SIM_TIME {
        ...
        if check_for_detailed_violation_mixed(&plan, &current_waypoints) {
            plan = replan(&mut agents, &grid, 2, seed, sim_time, mode);
...
fn main() {
    ...
    let modes = [InformationMode::Oracle, InformationMode::Nominal, InformationMode::Noisy];
    let map_size = 16;
    let num_unctrl = 8;
    let num_ctrl = 4;

    println!("Phase 4: Oracle & Information Quality ({} scenes, {}x{} grid, {}U/{}C)", 
             num_scenes, map_size, map_size, num_unctrl, num_ctrl);
    println!("{:<12} | {:<8} | {:<8} | {:<12}", 
             "Mode", "Success%", "Coll%", "Avg Replans");
    println!("{:-<12}-|-{:-<8}-|-{:-<8}-|-{:-<12}", 
             "", "", "", "");
    ...
    for &mode in &modes {
        ...
        for scene_seed in 0..num_scenes {
            let res = run_simulation(map_size, num_unctrl, num_ctrl, scene_seed as u64, mode);
            if res.success { successes += 1; }
            if res.collision { collisions += 1; }
            total_replans += res.num_replans;
        }

        let success_perc = (successes as f32 / num_scenes as f32) * 100.0;
        let coll_perc = (collisions as f32 / num_scenes as f32) * 100.0;
        let avg_replans = total_replans as f32 / num_scenes as f32;

        println!("{:<12?} | {:<7.1}% | {:<7.1}% | {:<12.2}", 
                 mode, success_perc, coll_perc, avg_replans);
    }
}
