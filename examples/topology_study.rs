...
fn generate_scenario(
    ...
    for _ in 0..num_uncontrolled {
        ...
        for _ in 0..100 { // Retry limit
            let start_r = rng.random_range(0..size);
            let start_c = rng.random_range(0..size);
            if grid[start_r][start_c] || occupied_cells.contains(&(start_r, start_c)) { continue; }
            
            let is_horizontal = rng.random_bool(0.5);
            let mut current_r = start_r;
            let mut current_c = start_c;
            track = vec![Vector2::new(current_c as f32, current_r as f32)];
            
            for _ in 0..size {
                if is_horizontal { current_c = (current_c + 1) % size; }
                else { current_r = (current_r + 1) % size; }
                if grid[current_r][current_c] { break; }
                track.push(Vector2::new(current_c as f32, current_r as f32));
            }
            if track.len() > 3 {
                for pos in &track {
    ...
    for r in 0..size {
        for c in 0..size {
            if !grid[r][c] { possible_cells.push((r, c)); }
        }
    }
    ...
    let mut pibt_grid = vec![vec![0; size]; size];
    for r in 0..size {
        for c in 0..size {
...
fn run_simulation(topology: Topology, size: usize, num_uncontrolled: usize, num_controlled: usize, seed: u64) -> SimulationResult {
    let (mut agents, grid) = generate_scenario(topology, size, num_uncontrolled, num_controlled, seed);
    if agents.len() < (num_uncontrolled + num_controlled) { return SimulationResult { collision: false, success: false, num_replans: 0 }; }
    ...
    while sim_time < MAX_SIM_TIME {
        ...
        if check_for_detailed_violation_mixed(&plan, &current_waypoints) {
            plan = replan(&mut agents, &grid, 2, seed, sim_time);
...
fn main() {
    ...
    let topologies = [Topology::Empty, Topology::Corridors, Topology::Warehouse];
    let map_size = 16;
    let num_unctrl = 8;
    let num_ctrl = 4;

    println!("Phase 3: Topological Diversity ({} scenes per topology, {}x{} grid, {}U/{}C)", 
             num_scenes, map_size, map_size, num_unctrl, num_ctrl);
    println!("{:<12} | {:<8} | {:<8} | {:<12}", 
             "Topology", "Success%", "Coll%", "Avg Replans");
    println!("{:-<12}-|-{:-<8}-|-{:-<8}-|-{:-<12}", 
             "", "", "", "");
    ...
    for &topo in &topologies {
        ...
        for scene_seed in 0..num_scenes {
            let res = run_simulation(topo, map_size, num_unctrl, num_ctrl, scene_seed as u64);
        ...
        println!("{:<12?} | {:<7.1}% | {:<7.1}% | {:<12.2}", 
                 topo, success_perc, coll_perc, avg_replans);
    }
}
